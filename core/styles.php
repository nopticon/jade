...
class style
{
	...
	function set_template($root = '.')
	{
		...
		$this->use_cache = $core->v('xs_use_cache');
		...
		if (!empty($this->tpl))
		{
			$replacement_file = $this->tpldir . $this->tpl . '/xs.cfg';
			
			if (@file_exists($replacement_file))
			{
				$replace = array();
				@include($replacement_file);
	...
	function compile_code($filename, $code)
	{
		...
		for ($i = 0; $i < $line_count; $i++)
		{
			...
			*/
			...
			{
				$params_str = preg_replace('#\{([a-z0-9\-_]*?)\}#ise', "isset_style_var('\\1')", $params_str);
	...
	*/
	function _compile_text($code)
	{
		...
		if (isset($this->vars['CSSPATH']))
		{
			$code = str_replace('CSSPATH', $this->vars['CSSPATH'], $code);
		}
	...
	function ext_function($var, $param = '')
	{
		...
		$var = strtolower($var);
		if (!class_exists('extf'))
		{
			//$extf = new html_extensions();
		}
		...
		if (!empty($param))
		{
			eval('$a = $core->' . $var . '(\'' . implode("', '", array_map('trim', explode(',', $param))) . '\');');
			return $a;
		}
	...
	*/
	...
	function add_config($tpl)
	{
		...
		global $core;
		$result = false;
		if (empty($core->config[$config_name]))
		{
	...
	*/
	function _get_config($tpl, $add_config)
	{
		...
		$this->style_config = $this->_unserialize($core->config[$config_name]);
		if ($tpl === $this->tpl)
		{
...
function isset_style_var($str)
{
	global $style;
	
	return isset($style->vars[$str]) ? $style->vars[$str] : $str;
}
